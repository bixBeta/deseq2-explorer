...
import {
  KNOWN_PLOTS, getAvailablePlots, captureSelectedPlots,
  buildPlotSectionsHtml, PLOT_HTML_EXTRAS,
...
function buildHtmlExport(md, sessionRows, contrasts, gseaRuns, alpha, plotSections = []) {
...
<section>
  ...
  // Build plot TOC entries from sections
  const plotTocItems = plotSections.map(sec => {
    const anchorId = `plot-${sec.anchorId ?? sec.id ?? sec.label.toLowerCase().replace(/\s+/g,'-')}`
    return { text: sec.label, href: `#${anchorId}` }
  })

  const allTocItems = [
    ...(plotTocItems.length ? [{ text: '— Plots —', href: null }, ...plotTocItems] : []),
    ...(tocItems.length     ? [{ text: '— Methods —', href: null }, ...tocItems]  : []),
  ]

  const tocSidebarHtml = allTocItems.length ? `
<nav class="toc-sidebar" id="toc-sidebar">
  <div class="toc-title">On this page</div>
  <ul>
    ${allTocItems.map(item => item.href
        ? `<li><a href="${item.href}" class="toc-link">${escHtml(item.text)}</a></li>`
        : `<li style="font-size:0.63rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#cbd5e1;padding:8px 8px 2px;list-style:none;">${escHtml(item.text)}</li>`
      ).join('\n    ')}
  </ul>
</nav>` : ''
...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  ${plotSections.length ? PLOT_HTML_EXTRAS.css : ''}
</style>
${plotSections.some(s => s.type === 'interactive') ? '<script src="https://cdn.plot.ly/plotly-2.27.0.min.js" charset="utf-8"><\/script>' : ''}
</head>
<body>
<div class="page-wrap">
  <div class="main-content">
    ...
    ${hasParams ? `${sessionHtml}${contrastsHtml}${gseaHtml}` : ''}
    ${plotSections.length ? `${hasParams ? '<hr>' : ''}${buildPlotSectionsHtml(plotSections)}<hr>` : (hasParams ? '<hr>' : '')}
    ${mdToHtml(md)}
...
<script>
${plotSections.length ? PLOT_HTML_EXTRAS.js : ''}
...
export default function ConsoleModal({ onClose, session, design, results, parseInfo, gseaRuns }) {
  ...
  const togglePlot = useCallback((id) => {
    setPlotSelected(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  }, [])
  const toggleAllPlots = useCallback((on) => {
  ...
  async function handleExport() {
    setExporting(true)
    setExportProg({ done: 0, total: 0, current: '' })

    let plotSections = []
    const hasPlots = KNOWN_PLOTS.some(p => plotSelected.has(p.id) && plotAvailable.has(p.id))
    ...
    const html = buildHtmlExport(methodsMd, sessionRows, contrasts, gseaRuns, alpha, plotSections)
    const blob = new Blob([html], { type: 'text/html' })
    const url  = URL.createObjectURL(blob)
    promptDownload('deseq2-explorer-report.html', name => {
  ...
  for (const p of KNOWN_PLOTS) {
    if (!plotGroups[p.group]) plotGroups[p.group] = []
    plotGroups[p.group].push(p)
  }
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            {exporting ? (
              <div style={{ display:'flex', flexDirection:'column', gap:3, width:180 }}>
                <div style={{ fontSize:'0.65rem', color:'var(--text-3)', lineHeight:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                  {exportProg.current || 'Building report…'}
                </div>
                <div style={{ height:3, borderRadius:2, background:'var(--border)', overflow:'hidden' }}>
                  <div style={{ height:'100%', borderRadius:2,
                    background:'linear-gradient(90deg,var(--accent),var(--accent2))',
                    width: exportProg.total ? `${(exportProg.done/exportProg.total)*100}%` : '15%',
              ...
              <button onClick={handleExport}
                ...
                ↓ Export HTML
                {selectedAvailable > 0 && (
                  <span style={{ marginLeft:5, fontSize:'0.65rem', opacity:0.75 }}>
        ...
        <div ref={scrollRef} style={{ flex:1, overflowY:'auto', padding:'20px 28px' }}>
          ...
          {tab === 'plots' && (
            <div style={{ paddingBottom: 8 }}>
              <p style={{ fontSize:'0.8rem', color:'var(--text-2)', marginBottom:20, lineHeight:1.6 }}>
                Choose which plots to include in the exported HTML report.
                They will appear as a <strong style={{ color:'var(--text-1)' }}>Plots</strong> section
                after the Methods content.
              </p>

              {/* Select all / none */}
              <div style={{ display:'flex', gap:8, marginBottom:20 }}>
                {[['Select all', true], ['Deselect all', false]].map(([label, on]) => (
                  <button key={label} onClick={() => toggleAllPlots(on)}
                    style={{ fontSize:'0.72rem', padding:'4px 12px', borderRadius:6,
              ...
              {Object.entries(plotGroups).map(([group, plots]) => (
                <div key={group} style={{ marginBottom:22 }}>
                  ...
                  {plots.map(p => {
                    const isAvail   = plotAvailable.has(p.id)
                    const isChecked = plotSelected.has(p.id)
                    ...
                    return (
                      <label key={p.id} style={{
                        ...
                        <input type="checkbox"
                          ...
                          onChange={() => isAvail && togglePlot(p.id)}
                          style={{ accentColor:'var(--accent)', width:14, height:14, flexShrink:0 }}
                        />
                        <span style={{ fontSize:'0.83rem', color:'var(--text-1)', flex:1 }}>{p.label}</span>
                        {hint && (
