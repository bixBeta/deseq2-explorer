...
const neg_log10 = p => (p != null && p > 0 ? -Math.log10(p) : 0)

export default function VolcanoPlot({ design, session, annMap }) {
  ...
  useRegisterPlot('volcano-plot', 'Volcano Plot', 'Differential Expression', captureRef)
  ...
  useEffect(() => {
    ...
    const negLog10Fdr   = neg_log10(fdr)
    ...
    const makeTrace = (pts, color, name, opacity, useWebGL = false) => ({
      x: pts.map(p => p.log2FC ?? 0),
      y: pts.map(p => neg_log10(p.padj)),
      mode: 'markers',
    ...
    const nsTrace = {
      ...makeTrace(ns, 'darkgray', nsLabel, 0.3, true),
      hoverinfo: 'skip',
    }
    ...
    const annotations = sig.map(p => ({
      ...
      arrowcolor: '#94a3b8',
      ax: 0, ay: -18,
      font: { size: 9, color: cssVar('--text-1') || '#1e293b' },
    ...
    const layout = {
      ...
      shapes: [
        // Vertical FC threshold lines
        {
          ...
          x0:  log2fc_thresh, x1:  log2fc_thresh,
        ...
        // Horizontal FDR threshold line
        {
          type: 'line', x0: 0, x1: 1, xref: 'paper',
          y0: negLog10Fdr, y1: negLog10Fdr,
          line: { color: '#94a3b8', dash: 'dot', width: 1 },
        },
      ],
    }
    ...
    const config = {
      ...
      autosize: true,
      displaylogo: false,
      toImageButtonOptions: { filename: 'volcanoplot', scale: 2, format: 'png' },
  ...
  if (!session?.sessionId) {
    return (
      <div style={{ padding: 60, textAlign: 'center', color: 'var(--text-3)', fontSize: '0.85rem' }}>
        Session not available — please re-run the analysis to view the volcano plot.
      </div>
    )
  }
